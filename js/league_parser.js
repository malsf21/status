...
function leagueFetch(summonerid){
  $.ajax({
      url: 'php/game.php?summonerid=' + summonerid,
      dataType: "json",
      success: function(league_game_data) {
        ...
        if ("status" in league_game_jsonData){
          ...
          for (i = 0; i < 10; i++){
            if (league_game_jsonData['participants'][i]["summonerId"] == summonerid){
      ...
      complete: function(xhr, data) {
        ...
        league_summoner_tier = league_summoner_jsonData[summonerid][0]["tier"];
        league_summoner_division = league_summoner_jsonData[summonerid][0]["entries"][0]["division"];
        leagueInfo();
      }
  });
}
